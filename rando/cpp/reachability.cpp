...
void compute_reachability(py::array_t<int16_t> graph_input,
    ...
    auto output_uc = output.mutable_unchecked<2>();
    ...
    assert(output_uc.shape(0) == num_vertices);
    assert(output_uc.shape(1) == NUM_RESOURCE_TYPES);
    ...
    for (int i = 0; i < num_vertices; i++) {
        for (int j = 0; j < NUM_RESOURCE_TYPES; j++) {
            output_uc(i, j) += vertex_best_value[i].resource_quantity[j];
        }
    }
}
