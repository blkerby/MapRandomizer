...
fn get_tech_by_difficulty(
    ...
    for (&tech_id, data) in tech_data_map.iter() {
        out.entry(data.difficulty.clone())
            .or_default()
            .push(tech_id);
    }
    out
...
impl PresetData {
    pub fn load(
        ...
        let tech_data: Vec<TechData> = serde_json::from_str(&tech_data_str)?;
        ...
        let notable_data: Vec<NotableData> = serde_json::from_str(&notable_data_str)?;
        let notable_data_map: HashMap<(RoomId, NotableId), NotableData> = notable_data
            .into_iter()
            .map(|x| ((x.room_id, x.notable_id), x))
            .collect();

        let difficulty_levels: Vec<String> = [
            "Implicit",
            "Basic",
            "Medium",
            "Hard",
        ...
        let skill_preset_names = difficulty_levels[1..8].to_vec();
        ...
        for name in skill_preset_names {
            ...
            let difficulty = DifficultyConfig::new(&preset, game_data);
        ...
        let tech_by_difficulty = get_tech_by_difficulty(&tech_data_map);
        let notables_by_difficulty = get_notables_by_difficulty(&notable_data_map);
