...
use maprando_game::{
    ...
    TECH_ID_CAN_SHINECHARGE_MOVEMENT, TECH_ID_CAN_SHINESPARK,
...
fn get_strat_difficulty(
    ...
    let mut best_difficulty_idx = preset_data.difficulty_levels.keys.len() - 1;
    ...
    if let Some(links) = links_by_ids.get(&key) {
        for link in links {
            ...
            let difficulty_idx = if link.difficulty as usize == preset_data.difficulty_tiers.len() {
                ...
                let difficulty_i =
                    preset_data.difficulty_tiers.len() - 1 - link.difficulty as usize;
                let difficulty = &preset_data.difficulty_tiers[difficulty_i];
                preset_data.difficulty_levels.index_by_key[&difficulty.name]
            };
            if difficulty_idx < best_difficulty_idx {
                best_difficulty_idx = difficulty_idx;
            }
        }
    }
    best_difficulty_idx
