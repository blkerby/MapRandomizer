...
class Network(torch.nn.Module):
    def __init__(self, room_tensor, map_x, map_y, global_map_channels, global_map_kernel_size, global_fc_widths,
        ...
        for i in range(len(global_map_channels) - 1):
            ...
            # global_map_layers.append(torch.nn.BatchNorm2d(map_channels[i + 1], momentum=batch_norm_momentum))
...
class TrainingSession():
    ...
    def generate_round(self, episode_length: int, num_candidates: int, temperature: float, render=False):
        ...
        with self.average_parameters.average_parameters():
            for j in range(episode_length):
                ...
                with torch.no_grad():
                    action_value = self.network(map, room_mask, candidate_placements, steps_remaining)
                action_probs = torch.softmax(action_value * temperature, dim=1)
                state_value = torch.sum(action_value * action_probs, dim=1)
    ...
    def train_round(self,
        ...
        for i in range(num_batches):
            ...
            action_value0 = self.network(map0_batch, room_mask0_batch, action_batch.unsqueeze(1),
                                                       steps_remaining_batch)
            action_value0 = action_value0[:, 0]
            loss = torch.mean((action_value0 - target_batch) ** 2)
...
for i in range(100000):
    ...
    logging.info(
        "{}: reward={:.2f} (max={:d}, cnt={:d}), state={:.4f}, action={:.4f}, mc_state={:.4f}, mc_action={:.4f}".format(
