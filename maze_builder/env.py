...
class MazeBuilderEnv:
    def __init__(self, rooms: List[Room], map_x: int, map_y: int, num_envs: int, must_areas_be_connected: bool, device, workers: int):
        self.device = device
    ...
    def compute_fast_component_matrix(self, room_mask, room_position_x, room_position_y):
        ...
        adjacency_matrix = torch.zeros_like(self.part_adjacency_matrix).unsqueeze(0).repeat(n, 1, 1)
        if adjacency_matrix.is_cuda:
            adjacency_matrix = adjacency_matrix.to(torch.float16)  # pytorch bmm can't handle integer types on CUDA
        for room_dir, room_dir_opp, part, part_opp in data_tuples:
            room_id = room_dir[:, 0]
        ...
        # A_list = []
        # total_time = 0
        A_out = torch.empty_like(good_matrix)
        def compute_component(i):
            undirected_E = torch.nonzero(good_matrix[i, :, :]).to(torch.uint8).to('cpu')
            # directed_E = torch.nonzero(good_matrix[i, :, :]).to(torch.uint8).to('cpu')
            # undirected_E = torch.zeros([0, 2], dtype=torch.uint8)
            root_mask = room_mask[i, self.good_part_room_id].to('cpu')
            ...
            A_out[i, :, :] = torch.maximum(A, self.good_base_matrix)
            # A_list.append(A)
        futures = []
        for i in range(num_envs):
            futures.append(self.executor.submit(compute_component, i))
        for fut in futures:
            fut.result()
        ...
        #     component_matrix = torch.bmm(component_matrix, component_matrix)
        #     component_matrix = torch.clamp_max(component_matrix, 1)
        # return component_matrix[:, :self.good_room_parts.shape[0], :self.good_room_parts.shape[0]].to(torch.bool), out
    ...
    def init_part_data(self):
        ...
        self.good_base_matrix = self.part_adjacency_matrix[self.good_room_parts.view(-1, 1), self.good_room_parts.view(1, -1)]
        non_loop_base_matrix = self.good_base_matrix.clone()
        non_loop_base_matrix[torch.arange(self.good_base_matrix.shape[0]), torch.arange(self.good_base_matrix.shape[0])] = 0
        self.directed_E = torch.nonzero(non_loop_base_matrix).to(torch.uint8).to('cpu')
