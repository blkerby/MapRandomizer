# From https://github.com/jettify/pytorch-optimizer/blob/master/torch_optimizer/shampoo.py
...
def _matrix_power(matrix: torch.Tensor, power: float) -> torch.Tensor:
    ...
    matrix = matrix.cpu()
    u, s, v = torch.svd(matrix.to(torch.float64))
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                for dim_id, dim in enumerate(grad.size()):
                    ...
                    grad_t = grad.t()
                    beta = group['beta']
                    precond = beta * precond + (1 - beta) * (grad @ grad_t)
                    state['precond_{}'.format(dim_id)] = precond
