...
// Same as above but we collapse open doors together (as if they were all connected with two-way edges)
// TODO: it may be more efficient to just compute reachability from/to the start location, instead
// of computing the condensation graph.
void compute_connectivity_collapsed(
        torch::Tensor room_mask,
        torch::Tensor room_position_x,
        torch::Tensor room_position_y,
        torch::Tensor room_left,
    ...
    // Add extra part for connecting open doors to each other:
    int special_part = num_parts;
    num_parts++;
        ...
        for (int g = start; g < end; g++) {
            ...
            for (int i = 0; i < room_right_proxy.size(0); i++) {
                ...
                if (it != position_to_part_map.end()) {
                    ...
                    adjacency[other_part].push_back(part);
                    adjacency[part].push_back(other_part);
                    position_to_part_map.erase(pos);
                } else {
                    adjacency[part].push_back(special_part);
                    adjacency[special_part].push_back(part);
                }
            }
            for (const auto &pair : position_to_part_map) {
                int part = pair.second;
                adjacency[part].push_back(special_part);
                adjacency[special_part].push_back(part);
            }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("compute_connectivity_collapsed", &compute_connectivity_collapsed,
          "compute a compressed representation of transitive closure of directed graphs"
          " assuming open doors are all connected to each other.");
